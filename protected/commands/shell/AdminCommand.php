...
EOD;
    ...
    public function run($args)
    {
        ...
        $module=Yii::app();
        $modelClass=$args[0];
        $this->templatePath = dirname(__FILE__) . '/views/admin';
        //lcfirst($modelClass) = $this->class2name($modelName);
    ...
    {
        return "CHtml::activeLabelEx(\$".lcfirst($modelClass).",'{$column->name}')";
    }

    public function generateInputField($modelClass,$column)
    {
        if($column->type==='boolean')
            return "CHtml::activeCheckBox(\$".lcfirst($modelClass).",'{$column->name}')";
        else if(stripos($column->dbType,'text')!==false)
            return "CHtml::activeTextArea(\$".lcfirst($modelClass).",'{$column->name}',array('rows'=>6, 'cols'=>50))";
        else
        {
            ...
            if($column->type!=='string' || $column->size===null)
                return "CHtml::{$inputField}(\$".lcfirst($modelClass).",'{$column->name}')";
            else
            {
                if(($size=$maxLength=$column->size)>60)
                    $size=60;
                return "CHtml::{$inputField}(\$".lcfirst($modelClass).",'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
            }
        }
    }
    ...
    {
        return "\$form->labelEx(\$".lcfirst($modelClass).",'{$column->name}')";
    }
    ...
    public function generateActiveField($modelClass,$column)
    {
        ...
        foreach($model->relations() as $relation){
            if($relation[0] == CActiveRecord::BELONGS_TO && $relation[2] == $column){
                return "echo \$form->DropDownList(\$".lcfirst($modelClass).",'{$relation[2]}',
                                CHtml::listData({$relation[1]}::model()->findAll(), 'id', 'nama'))";
            }
        }
        
    
        if($column->type==='boolean')
            return "echo \$form->checkBox(\$".lcfirst($modelClass).",'{$column->name}')";
        else if(stripos($column->dbType,'text')!==false)
            return "echo \$form->textArea(\$".lcfirst($modelClass).",'{$column->name}',array('rows'=>6, 'cols'=>50))";
        else if(strtoupper($column->dbType) == 'DATE'){
            return "\$form->widget('zii.widgets.jui.CJuiDatePicker',array(
                                'model'=>\$".lcfirst($modelClass).",
                                'name'=>'{$modelClass}[{$column->name}]',
                                'value'=>\$".lcfirst($modelClass)."->{$column->name},
                                'options'=>array(
                                    'dateFormat' => 'yy-mm-dd',
                                    'changeYear' => true,
                                    'yearRange' => '1950:2000'
            ...
            if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name)){
                ...
                return "echo \$form->{$inputField}(\$".lcfirst($modelClass).",'{$column->name}')";
            } else {
                if(($size=$maxLength=$column->size)>60)
                    $size=60;
                return "echo \$form->{$inputField}(\$".lcfirst($modelClass).",'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
            }
        }
    }
