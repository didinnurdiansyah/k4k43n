...
class CrudCode extends CCodeModel
{
    ...
    {
        return "CHtml::activeLabelEx(\${$this->modelId},'{$column->name}')";
    }

    public function generateInputField($modelClass,$column)
    {
        if($column->type==='boolean')
            return "CHtml::activeCheckBox(\${$this->modelId},'{$column->name}')";
        else if(stripos($column->dbType,'text')!==false)
            return "CHtml::activeTextArea(\${$this->modelId},'{$column->name}',array('rows'=>6, 'cols'=>50))";
        else
        {
            ...
            if($column->type!=='string' || $column->size===null)
                return "CHtml::{$inputField}(\${$this->modelId},'{$column->name}')";
            else
            {
                if(($size=$maxLength=$column->size)>60)
                    $size=60;
                return "CHtml::{$inputField}(\${$this->modelId},'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
            }
        }
    }
    ...
    {
        return "\$form->labelEx(\${$this->modelId},'{$column->name}')";
    }

    public function generateActiveField($modelClass,$column)
    {
        if($column->type==='boolean')
            return "\$form->checkBox(\${$this->modelId},'{$column->name}')";
        else if(stripos($column->dbType,'text')!==false)
            return "\$form->textArea(\${$this->modelId},'{$column->name}',array('rows'=>6, 'cols'=>50))";
        else
        {
            if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
            ...
            if($column->type!=='string' || $column->size===null)
                return "\$form->{$inputField}(\${$this->modelId},'{$column->name}')";
            else
            {
                if(($size=$maxLength=$column->size)>60)
                    $size=60;
                return "\$form->{$inputField}(\${$this->modelId},'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
            }
        }
    }
